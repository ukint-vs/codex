...
async fn test_full_cross_chain_cycle_simulation() {
    ...
    remoting.system().mint_to(eth_bridge_id, 100_000_000_000_000);
    service_client
    ...
    user_service.vault_withdraw(user_actor, token_addr.into(), amount).await.unwrap();
    ...
    async fn test_force_exit_cancellation_simulation() {
        ...
        let eth_remoting = remoting.clone().with_actor_id(eth_bridge_id);
        let mut eth_service =
            Service::<VaultImpl, _>::new(eth_remoting, program_id, "Vault".into());
        eth_service.eth_deposit(deposit_data.abi_encode()).await.unwrap();
        ...
        service_client.vault_force_exit(user_actor, token_addr.into(), amount).await.unwrap();
