...
//     vault
//         .vault_deposit(vc(buyer()), usdt_micro(10_000))
//         .with_actor_id(ADMIN_ID.into())
//         .await
//         .unwrap();
//     vault
//         .transfer_to_market(vc(orderbook_program.id().into()), usdt_micro(10_000))
...
//     let (avail, _reserved) = vault.get_balance(vc(buyer())).await.unwrap();
