...
import { VAULT_WASM_PATH } from './config.ts';

async function main() {
  const wasmPath = path.resolve(VAULT_WASM_PATH);
  const wasmCode = readFileSync(wasmPath);

  console.log('\nUploading Vault WASM via ethexe CLI...');
  ...
  console.log('\n✓ Vault Code validated');
  console.log('Code ID:', codeId);
  console.log('\nAdd to .env: VAULT_CODE_ID=' + codeId);
}
