...
import { ORDERBOOK_CODE_ID, SALT } from './config.ts';

function normalizeCodeId(codeId: string): `0x${string}` {
  const hex = codeId.startsWith('0x') ? codeId : `0x${codeId}`;
  if (hex.length !== 66) {
    throw new Error('ORDERBOOK_CODE_ID must be 32-byte hex (0x + 64 chars)');
  }
...
async function main() {
  if (!ORDERBOOK_CODE_ID) {
    console.error('Error: ORDERBOOK_CODE_ID not set in .env');
    process.exit(1);
  }

  const codeId = normalizeCodeId(ORDERBOOK_CODE_ID);
  const obSalt = `${SALT}_ob`;
  const saltHex = toSaltH256(obSalt);

  console.log('\nCreating Orderbook via ethexe CLI...');
  ...
  console.log('\n✓ Done! Add to .env: ORDERBOOK_PROGRAM_ID=' + programId);
}
